...
class Central:
    ...
    async def get_player_kudos_stats(self, discord_id: int) -> dict:
        """Получает детальную статистику кудосов игрока по Discord ID."""
        endpoint = f"{self.endpoint}/api/v1/kudos/stats/discord/{discord_id}"
        ...
        async with ClientSession() as session:
            async with session.get(endpoint, headers={"Authorization": f"Bearer {self.bearer_token}"}) as response:
                if response.status == 404:
                    return None
                if response.status != 200:
                    raise Exception(f"Failed to get kudos stats: {response.status} - {await response.text()}")
                
                return await response.json()

    async def get_kudos_rating(self, limit: int = 10) -> list[dict]:
        """Получает ТОП игроков по репутации."""
        endpoint = f"{self.endpoint}/api/v1/kudos/rating"
        params = {"limit": limit}
        
        async with ClientSession() as session:
            async with session.get(endpoint, params=params, headers={"Authorization": f"Bearer {self.bearer_token}"}) as response:
                if response.status != 200:
                    raise Exception(f"Failed to get kudos rating: {response.status} - {await response.text()}")
                
                # Если твой API возвращает список напрямую (как мы видели в браузере [])
                return await response.json()

    async def admin_check_kudos(self, ckey: str) -> list[dict]:
        """Получает историю похвал для админ-команды."""
        endpoint = f"{self.endpoint}/api/v1/kudos/check/{ckey}"
        ...
        async with ClientSession() as session:
            async with session.get(endpoint, headers={"Authorization": f"Bearer {self.bearer_token}"}) as response:
                if response.status == 404:
                    return []
                if response.status != 200:
                    raise Exception(f"Failed to check kudos logs: {response.status} - {await response.text()}")
